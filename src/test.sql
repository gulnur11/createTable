...
create type FamilyStatus as enum('Single', 'Married','Divorced', 'Separated', 'Widow', 'Other');
...
alter table agencies
    add constraint check_number
        check (phone_number like '+996%' and LENGTH(phone_number) = 13);

alter table rent_info add column check_in date;

alter table rent_info add column check_out date;

alter table owners add constraint un unique (email);

alter table customers add constraint u unique (email);
...
insert into agencies (name, phone_number, address_id) values ('Apartment Agency', '+996550123456', 2);
insert into agencies (name, phone_number, address_id) values ('Rent Agency', '+996999098765', 3);
--Бардык уйлорду, уйдун ээсинин атын, клиенттин атын, агенттин атын чыгар.
...
SELECT o.first_name,
       ...
       h.description,
       h.price,
       h.rating FROM owners o
         JOIN houses h ON o.id = h.owner_id
WHERE h.house_type = 'House' ORDER BY LENGTH(o.first_name) DESC LIMIT 1;

-- 2. Уйлордун баалары 1500, 2000 дин аралыгында бар болсо true чыгар, жок болсо false чыгар.
SELECT CASE WHEN EXISTS ( SELECT 1 FROM houses WHERE price BETWEEN 1500 AND 2000 )
...
SELECT
    ...
    h.price,
    h.rating
FROM addresses a LEFT JOIN houses h ON a.id = h.address_id WHERE a.id IN (5, 6, 7, 8, 9, 10);
...
SELECT
    ...
    gender,
    nationality,
    marital_status FROM customers WHERE date_of_birth > '1999-12-31' ORDER BY date_of_birth ASC LIMIT 15 OFFSET 10;
...
SELECT
    ...
    ad_agency.city AS agency_city,
    ad_agency.region AS agency_region,
    ad_agency.street AS agency_street,
    h.house_type,
    h.price,
    h.rating,
    ad_house.city AS house_city,
    ad_house.region AS house_region,
    ad_house.street AS house_street
FROM agencies a
         LEFT JOIN addresses ad_agency ON a.address_id = ad_agency.id
         LEFT JOIN houses h ON a.id = h.address_id
         LEFT JOIN addresses ad_house ON h.address_id = ad_house.id
...
FROM houses h
         LEFT JOIN addresses ad_house ON h.address_id = ad_house.id
         LEFT JOIN agencies a ON h.owner_id = a.id
WHERE NOT EXISTS (
    SELECT 1
    FROM rent_info ri
    WHERE ri.house_id = h.id
);

-- 11.Клиенттердин улутуна карап, улутуну жана ал улуутта канча клиент жашайт санын чыгар.
SELECT
    nationality,
    COUNT(*) AS number_of_clients FROM customers GROUP BY nationality;
